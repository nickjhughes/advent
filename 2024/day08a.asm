...
parse_char:
  ...
  cmp $46, %cl # '.'
  je blank
  # Antenna here, so store char in antenna map and 0x00 in antinode map
  movb %cl, (%r12,%r10,1)
  movb $0, (%r13,%r10,1)
  inc %r10
  jmp parse_char
...
# Find the two antinodes for the antennas at indices %r10 and %r11 in the map. If they're inside
...
calc_antinodes:
  ...
  # Find two antinodes
  # First is (row, col) = (%rbx-%rax,%rcx-%rdx)
  sub %rax, %rbx
  sub %rdx, %rcx
  # Bounds check
  cmp $0, %rbx
...
second_antinode:
  # Second is (row, col) = (%rdi+%rax,%rsi+%rdx)
  add %rax, %rdi
  add %rdx, %rsi
  # Bounds check
  cmp $0, %rdi
